...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  load(assetUri, startTime, mimeType) {
    ...
    events.onStart =
        () => shaka.log.info('events.onStart load of ' + assetUri + '...');
    return new Promise((resolve, reject) => {
      shaka.log.info(
          'starting promise, attaching events listeners (player.js: 1072)'
      );
      ...
      events.onEnd = () => {
        shaka.log.info('events.onEnd (player.js:1081)');
        resolve();
      ...
      events.onCancel = () => {
        shaka.log.info('events.onCancel (player.js:1087)');
        shaka.log.error();
        reject(this.createAbortLoadError_());
      };
      events.onError = (e) => {
        shaka.log.info('events.onError (player.js:1092)');
  ...
  static filterForAVVariants_(manifest) {
    ...
    if (manifest.variants.some(isAVVariant)) {
      ...
      shaka.log.debug('No varaint found');
    }
  }
